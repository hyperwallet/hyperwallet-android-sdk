...
public class DeactivatePaperCheckTest {
    ...
    @Test
    public void testDeactivatePaperCheck_successfulStatusTransition() throws Exception {

        String responseBody = mExternalResourceManager.getResourceContent(
                "deactivated_paper_check_response.json");
        mServer.mockResponse().withHttpResponseCode(HttpURLConnection.HTTP_CREATED).withBody(responseBody).mock();

        Hyperwallet.getDefault().deactivatePaperCheck("trm-fake-token", null,
                mListener);
        ...
        assertThat(statusTransitionResponse.getNotes(), is("Closing PaperCheck."));
        ...
        assertThat(request.getPath(), endsWith(
                "users/test-user-token/paper-checks/trm-fake-token/status-transitions"));
        assertThat(request.getMethod(), is(POST.name()));
    }

    @Test
    public void testDeactivatePaperCheck_unsuccessfulStatusTransition() throws Exception {

        String responseBody = mExternalResourceManager.getResourceContentError("invalid_status_transition.json");
        mServer.mockResponse().withHttpResponseCode(HttpURLConnection.HTTP_BAD_REQUEST).withBody(responseBody).mock();

        Hyperwallet.getDefault().deactivatePaperCheck("trm-fake-token", null, mListener);
