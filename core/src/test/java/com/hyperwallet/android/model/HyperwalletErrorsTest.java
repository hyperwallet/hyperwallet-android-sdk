...
public class HyperwalletErrorsTest {

    private static final HyperwalletError FORBIDDEN_HYPERWALLET_ERROR = new HyperwalletError(
            "The caller does not have access to the requested resource", "FORBIDDEN");
    private static final HyperwalletError UNAUTHORIZED_HYPERWALLET_ERROR = new HyperwalletError(
            "Please check your login credentials and try again", "INCORRECT_LOGIN_CREDENTIALS");
    private static final HyperwalletError RELATED_RESOURCE_HYPERWALLET_ERROR = new HyperwalletError(
    ...
    private static final HyperwalletError INVALID_BRANCH_ID_ERROR = new HyperwalletError(
    ...
    public void testFromJsonString_forbiddenErrorResponse() throws Exception {
        HyperwalletErrors errors = fromJsonString(mResourceManager
    ...
    public void testFromJsonString_constraintErrorResponse() throws Exception {
        HyperwalletError patternValidationError = new HyperwalletError(
                "Account Number is invalid. The maximum length of this field is 17.",
                "bankAccountId",
                "CONSTRAINT_VIOLATIONS");

        HyperwalletError presenceValidationError = new HyperwalletError(
                "Routing Number: You must provide a value for this field",
                "branchId",
                "CONSTRAINT_VIOLATIONS");

        List<HyperwalletError> expectedErrorsList = new ArrayList<>();
        expectedErrorsList.add(patternValidationError);
        expectedErrorsList.add(presenceValidationError);

        HyperwalletErrors expectedErrors = new HyperwalletErrors(expectedErrorsList);
        HyperwalletErrors actualErrors = fromJsonString(mResourceManager
    ...
    public void testHyperwalletError_isParcelable() throws Exception {
        ...
        HyperwalletError error = actualErrors.getErrors().get(0);
        ...
        HyperwalletError bundledError = HyperwalletError.CREATOR.createFromParcel(parcel);
    ...
    public void testContainsInputError_withFieldInputError() {
        HyperwalletErrors hyperwalletErrors = new HyperwalletErrors(Arrays.asList(INVALID_BRANCH_ID_ERROR));
    ...
    public void testContainsInputError_noFieldInputError() {
        HyperwalletErrors hyperwalletErrors = new HyperwalletErrors(
